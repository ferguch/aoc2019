...
def locations(path: str) -> Set[Coord]:
    x = 0
    y = 0
    visited = set()
    ...
    for line in path.split(","):
        ...
        for i in range(distance):
            ...
            if direction == "U":
                ...
                raise ValueError("Unknown direction error")
            visited[Coord(x, y)] = num_steps
    return visited

def intersection_steps(path1: str, path2: str) -> int:
    dist1 = steps(path1)
    dist2 = steps(path2)
    intersects = set(dist1).intersection(set(dist2))
    print(intersects)
    min_steps = min(dist1[loc] + dist2[loc] for loc in intersects)
    return min_steps

assert intersection_steps("R8,U5,L5,D3", "U7,R6,D4,L4") == 30
assert intersection_steps("R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83") == 610
assert intersection_steps("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7") == 410
steps = intersection_steps(P1, P2)
print(steps)